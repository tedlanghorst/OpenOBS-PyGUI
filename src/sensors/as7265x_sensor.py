...
class AS7265XSensor(BaseSensor):
    ...
    def configure_gui(self, parent_frame):
        ...
        ttk.Combobox(
            setting_frame, values=[12.5, 25, 50, 100], textvariable=self.led_current_var
        ).grid(row=0, column=1, sticky="ew")

        ttk.Label(setting_frame, text="Gain:").grid(row=1, column=0, sticky="w")
        ttk.Combobox(
            setting_frame, values=[1, 3.7, 16, 64], textvariable=self.gain_var
        ).grid(row=1, column=1, sticky="ew")

        ttk.Label(setting_frame, text="Integration Cycles:").grid(
            row=2, column=0, sticky="w"
        )
        ttk.Entry(setting_frame, textvariable=self.integration_cycles_var).grid(
