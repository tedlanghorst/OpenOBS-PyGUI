...
class TimeSeriesPlot(BasePlot):
    ...
    def _setup_controls(self):
        self.time_series_listbox = tk.Listbox(self.controls_frame, selectmode='multiple', height=1)
        self.time_series_listbox.grid(row=0, column=0)

        # Bind the selection change event to the update function using a lambda
        self.time_series_listbox.bind('<<ListboxSelect>>', lambda event: self.update(None))

        # Clear selections button
        ttk.Button(self.controls_frame,
                   text="Clear Selections",
                   command=lambda: self.time_series_listbox.selection_clear(0, tk.END)).grid(row=1, column=0)
        ...
        tk.Button(n_samples_frame, text="Submit", command=self._get_num_samples).grid(row=2, column=0)
    ...
    def update(self, data):
        ...
        if data:
            ...
            if len(self.df) == 0:
                ...
                self.lines = [self.ax.plot([], [], linewidth=2, label=col)[0] for col in self.df.columns]
            else:
